...
class TransactionController extends Controller
{
    ...
    public function postSend(Request $request)
    {
        ...
        //print_r($sender_account);die();
        $balance=$sender_account->balance;
        $balance_usd=$sender_account->balance_usd;
        $balance_gpb=$sender_account->balance_gbp;
        $balance=(float)$balance;
        $balance_usd=(float)$balance_usd;
        $balance_gpb=(float)$balance_gpb;
    ...
    public function cardDeposit(Request $request)
    {
        ...
        $transaction->fee='0';
        $transaction->client_id=$id =auth()->guard('client')->user()->id;
        $transaction->sender='Besjana Roko';
        $transaction->status='completed';
